...
# Load config if it exists; otherwise bootstrap a minimal one
if [[ -f /etc/vps-config ]]; then
    source /etc/vps-config
else
    warn "/etc/vps-config not found — running in standalone mode."
    # Auto-detect domain from hostname / rdns, or ask
    AUTO_DOMAIN=$(hostname -f 2>/dev/null | grep '\.' || true)
    if [[ -z "$AUTO_DOMAIN" ]]; then
        read -rp "  Enter your domain (e.g. cloud.superdbot.com): " AUTO_DOMAIN
    fi
    DOMAIN="${AUTO_DOMAIN:-cloud.superdbot.com}"
    IP=$(curl -s4 ifconfig.me 2>/dev/null || hostname -I | awk '{print $1}')
    # Generate fresh UUIDs/passwords
    VMESS_UUID=$(cat /proc/sys/kernel/random/uuid)
    VLESS_UUID=$(cat /proc/sys/kernel/random/uuid)
    TROJAN_PASS=$(openssl rand -hex 12)
    SS_PASS=$(openssl rand -hex 12)
    # Write config so future scripts can source it
    mkdir -p /etc
    cat > /etc/vps-config <<CFG
DOMAIN=${DOMAIN}
IP=${IP}
...
SS_PASS=${SS_PASS}
CFG
    log "Created /etc/vps-config  (domain=${DOMAIN}  ip=${IP})"
fi

# ════════════════════════════════════════════════════════════════
step "0 — Install required packages (safe to skip if already installed)"
# ════════════════════════════════════════════════════════════════
export DEBIAN_FRONTEND=noninteractive
apt-get update -qq
apt-get install -y -qq \
    gcc netcat-openbsd curl wget openssl \
    nginx dropbear squid \
    psmisc net-tools 2>/dev/null

# Install XRay if missing
if ! command -v xray &>/dev/null; then
    warn "XRay not found — installing..."
    bash -c "$(curl -fsSL https://github.com/XTLS/Xray-install/raw/main/install-release.sh)" @ install \
        -u root 2>/dev/null && log "XRay installed" || err "XRay install failed"
fi

# Install BadVPN UDP gateway if missing
if ! command -v badvpn-udpgw &>/dev/null; then
    wget -qO /usr/local/bin/badvpn-udpgw \
        ...
        2>/dev/null && chmod +x /usr/local/bin/badvpn-udpgw && log "BadVPN installed"
fi

# SSL cert for XRay/NGINX
mkdir -p /etc/xray/ssl
if [[ ! -f /etc/xray/ssl/cert.crt ]]; then
    openssl req -newkey rsa:2048 -nodes -x509 -days 3650 \
        -keyout /etc/xray/ssl/private.key \
        -out    /etc/xray/ssl/cert.crt \
        -subj   "/CN=${DOMAIN}" 2>/dev/null && log "Self-signed SSL cert created"
fi
